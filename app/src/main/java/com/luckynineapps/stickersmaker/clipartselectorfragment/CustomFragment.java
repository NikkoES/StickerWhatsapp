    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == SELECT_EMOTICON) {
                File clipArtFile = new File(activity.getFilesDir(), UUID.randomUUID().toString()+".png");
                Tool.extractURIToFile(activity, data.getData(), clipArtFile);
                Intent i = new Intent();
                i.putExtra("clip_art_path", clipArtFile.getAbsolutePath());
